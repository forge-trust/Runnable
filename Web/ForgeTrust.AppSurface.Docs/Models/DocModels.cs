    ...
    IReadOnlyList<DocSymbolSourceProvenance>? SymbolSourceProvenance = null)
{
    ...
    /// </summary>
    /// <remarks>
    /// These server-issued tokens are consumed only by the rich-authoring client enhancement. They remain a public
    /// non-positional property so existing authors using the legacy HTML bridge retain the original document contract.
    /// </remarks>
...
public sealed record DocDetailsViewModel
{
    ...
    /// <summary>
    /// Gets a value indicating whether the page is a C# API reference document for presentation purposes.
    /// This preserves the established classification for legacy HTML-only C# documents as well as typed namespace
    /// pages. It does not select a renderer; <see cref="CSharpRenderKind"/> is the sole internal render selector.
    /// </summary>
